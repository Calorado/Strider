	...
	public:
					...
					if (sector == 0) {
						...
						if (numberSectors == 1) {
                            if (sectorSize < 65536)
                                ...
                                dataSectors[sector].pbMask = std::min(dataSectors[sector].pbMask, (size_t)3);
							break;
						}
						else {
		...
		int calculate_lc(const uint8_t* input, const size_t size, const uint8_t* freqCostTable) {
			...
			for (size_t i = 0; i < numberSectors; i++) {
				...
				//printf("\n S %d %d", dataSectors[i].pbMask, dataSectors[i].lcValue);
			}
